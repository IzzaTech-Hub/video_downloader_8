...
class _MyWebViewState extends State<MyWebView> {
  ...
  // void setUserAgent() {
  //   if (controller.searchTextCTL.text.contains("facebook.com") ||
  //       controller.searchTextCTL.text.contains("fb")) {}
  ...
  void setUserAgent() {
    int round = getRandomNumber(UserAgents.userAgentsListOldDevices.length);
    options.crossPlatform.userAgent =
        "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/40.0.2214.115 Safari/537.36";
    // UserAgents.userAgentsListOldDevices[round];

    print("User Agent: ${options.crossPlatform.userAgent}");
    setState(() {});
    // if (controller.searchTextCTL.text.contains("facebook.com") ||
    //     controller.searchTextCTL.text.contains("fb")) {
    //   // Do nothing if the URL contains "facebook.com" or "fb".
    //   int round = getRandomNumber(UserAgents.userAgentsListOldDevices.length);
    //   options.crossPlatform.userAgent =
    //       UserAgents.userAgentsListOldDevices[round];
    // } else {
    //   int round = getRandomNumber(UserAgents.userAgentsList2.length);
    //   options.crossPlatform.userAgent =
    //       UserAgents.userAgentsListOldDevices[round];
    // }
  }
    ...
    return Scaffold(
      body: Container(
        ...
        child: InAppWebView(
          key: webViewKey,
          initialUrlRequest:
              URLRequest(url: WebUri(controller.searchTextCTL.text)
              // Uri.parse(controller.searchTextCTL.text)
              ),
        ...
        """;
          ...
          onProgressChanged: (thisController, progress) async {
            ...
            if (progress > 50) {
              controller.CompleteHTML(thisController);
              print("Current Page: ${controller.currentPage}");
              ...
              if (controller.currentPage.contains("intent:")) {
                ...
                thisController.loadUrl(
                    urlRequest: URLRequest(url: WebUri(changedUrl)
                    //  Uri.parse(changedUrl)
                    ));
                controller.CheckShareChatVideoURLFromWeb(thisController);
              }
            }
